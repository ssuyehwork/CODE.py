        ...
        try:
            ...
            if filters:
                if filters.get('is_favorite'):
                    query = query.filter(ClipboardItem.is_favorite == True)
                if filters.get('is_locked'):
                    query = query.filter(ClipboardItem.is_locked == True)

            if advanced_filters:
                if 'star_min' in advanced_filters and 'star_max' in advanced_filters:
                    query = query.filter(ClipboardItem.star_level.between(advanced_filters['star_min'], advanced_filters['star_max']))
                if 'date_start' in advanced_filters and 'date_end' in advanced_filters:
                    # a new day starts at 00:00:00 and ends at 23:59:59
                    from datetime import time, datetime
                    start_datetime = datetime.combine(advanced_filters['date_start'], time.min)
                    end_datetime = datetime.combine(advanced_filters['date_end'], time.max)
                    query = query.filter(ClipboardItem.created_at.between(start_datetime, end_datetime))

            if selected_tags:
                query = query.join(item_tags).join(Tag).filter(Tag.name.in_(selected_tags))
...
# === 4. 高级筛选对话框 ===
from PyQt5.QtWidgets import QDialogButtonBox, QDateEdit
...
class AdvancedFilterDialog(QDialog):
    def __init__(self, parent=None):
        ...
        star_layout.addWidget(QLabel("⭐ 星级范围:"))
        self.star_min = QSpinBox()
        self.star_min.setRange(0, 5)
        self.star_max = QSpinBox()
        self.star_max.setRange(0, 5)
        self.star_max.setValue(5)
        star_layout.addWidget(self.star_min)
        star_layout.addWidget(QLabel("到"))
        star_layout.addWidget(self.star_max)
        layout.addLayout(star_layout)

        # 日期筛选
        date_layout = QGridLayout()
        date_layout.addWidget(QLabel("📅 创建日期范围:"), 0, 0, 1, 2)
        ...
        date_layout.addWidget(self.date_start, 1, 1)
        date_layout.addWidget(QLabel("到:"), 2, 0)
        date_layout.addWidget(self.date_end, 2, 1)
...
class ClipboardApp(QMainWindow):
    def __init__(self):
        ...
        self.advanced_filters = {} # 存储高级筛选条件
    ...
    def init_top_bar(self):
        ...
        self.btn_all_filter = QPushButton("全部")
        self.cb_fav_filter = QCheckBox("收藏")
        self.cb_lock_filter = QCheckBox("锁定")

        self.btn_all_filter.clicked.connect(self.clear_filters)
        self.cb_fav_filter.stateChanged.connect(self.load_data)
        self.cb_lock_filter.stateChanged.connect(self.load_data)

        layout.addWidget(self.btn_all_filter)
        layout.addWidget(self.cb_fav_filter)
        layout.addWidget(self.cb_lock_filter)
    ...
    def clear_filters(self):
        ...
        # blockSignals防止触发多次load_data
        self.cb_fav_filter.blockSignals(True)
        self.cb_lock_filter.blockSignals(True)

        self.cb_fav_filter.setChecked(False)
        self.cb_lock_filter.setChecked(False)

        self.cb_fav_filter.blockSignals(False)
        self.cb_lock_filter.blockSignals(False)
    ...
    def load_data(self, select_id=None):
        ...
        # 收集筛选条件
        filters = {
            'is_favorite': self.cb_fav_filter.isChecked(),
            'is_locked': self.cb_lock_filter.isChecked(),
        }

        search = self.search_input.text().strip()
        sort = self.sort_map.get(self.sort_combo.currentIndex(), "manual")
        items = self.db.get_all_items(filters, search, sort, self.selected_tags, self.advanced_filters)
    ...
    def refresh_tag_cloud(self):
        ...
        for name, count in tags_data:
            ...
            if name in self.selected_tags:
                btn.setChecked(True)
                btn.setStyleSheet("background-color: #89b4fa; color: #1e1e2e; border: 1px solid #89b4fa;")

            btn.clicked.connect(lambda _, t=name: self.apply_tag_filter(t))
    ...
    def apply_tag_filter(self, tag_name):
        ...
        if modifiers == Qt.ControlModifier:
            ...
            # Click: 单选或取消单选
            if self.selected_tags == {tag_name}:
                self.selected_tags.clear()
            else:
                self.selected_tags = {tag_name}
    ...
    def group_selected_items(self):
        """智能颜色标签切换：根据选中项の颜色状态，智能添加或取消颜色标签"""
        selected_rows = self.table.selectionModel().selectedRows()
    ...
    def keyPressEvent(self, event):
        ...
        # 移除KeypadModifier，以便统一处理主键盘和数字小键盘
        core_modifiers = modifiers & ~Qt.KeypadModifier

        # 检查是否只按下了Ctrl键 (允许附带KeypadModifier)
        if core_modifiers == Qt.ControlModifier:
            star_level = -1
    ...
    def save_window_state(self):
        ...
        # 保存UI状态
        settings.setValue("favFilterChecked", self.cb_fav_filter.isChecked())
        settings.setValue("lockFilterChecked", self.cb_lock_filter.isChecked())
    ...
    def restore_window_state(self):
        ...
        # 恢复UI状态
        fav_checked = settings.value("favFilterChecked", False, type=bool)
        lock_checked = settings.value("lockFilterChecked", False, type=bool)
        self.cb_fav_filter.setChecked(fav_checked)
        self.cb_lock_filter.setChecked(lock_checked)
