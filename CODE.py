...
class ClipboardApp(QMainWindow):
    ...
    def run_self_test_and_screenshot(self):
        """
        内部集成测试函数：
        1. 模拟剪贴板操作 (文本, 图片, 文件)
        2. 刷新UI
        ...
        4. 自动退出
        """
        print("--- [Internal Test] Starting self-test and screenshot sequence... ---")

        # 使用QTimer来确保每个操作都能被主事件循环处理
        from PyQt5.QtCore import QTimer

        def step_1_copy_text():
            print("[Internal Test] Step 1: Simulating text copy.")
            self.monitor_enabled = False
            self.clipboard.setText("Hello from internal test!")
            self.monitor_enabled = True
            self.on_clipboard_change() # Manually trigger
            QTimer.singleShot(500, step_2_copy_image) # Wait 500ms

        def step_2_copy_image():
            print("[Internal Test] Step 2: Simulating image copy.")
            image = QImage(100, 100, QImage.Format_RGB32)
            image.fill(QColor("#89b4fa")) # Blue color
            self.monitor_enabled = False
        ...
        def step_3_copy_file():
            print("[Internal Test] Step 3: Simulating file copy.")
            # Create a dummy file
            self.test_file_path = os.path.abspath("internal_test_file.txt")
            ...
            self.monitor_enabled = True
            self.on_clipboard_change()
            QTimer.singleShot(1500, step_4_screenshot_and_exit) # Wait longer for UI to update

        def step_4_screenshot_and_exit():
            print("[Internal Test] Step 4: Taking screenshot and exiting.")
            # Ensure table is updated by reloading data
            self.load_data()
            QApplication.processEvents() # Process any pending events to make sure UI is updated

            # Take screenshot
            pixmap = self.grab()
            pixmap.save("final_verification.png")
            print("[Internal Test] Screenshot 'final_verification.png' saved.")

            # Clean up and exit
            if hasattr(self, 'test_file_path') and os.path.exists(self.test_file_path):
                os.remove(self.test_file_path)

            print("--- [Internal Test] Test finished. Exiting. ---")
            QApplication.instance().quit()

        # Start the test sequence
        QTimer.singleShot(1000, step_1_copy_text) # Initial delay for app to fully load
...
if __name__ == "__main__":
    ...
    # 如果是测试模式，则运行自测试并截图
    if is_test_mode:
        window.run_self_test_and_screenshot()
