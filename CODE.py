...
class ClipboardApp(QMainWindow):
    ...
    def run_robust_self_test(self):
        ...
        def step_3_copy_file():
            ...
            self.on_clipboard_change()
            _select_top_row_and_process_events()
            QTimer.singleShot(500, step_4_verify_color_set)

        def step_4_verify_color_set():
            print("[Robust Self-Test] Step 4: Verifying color setting.")
            if self.table.rowCount() == 0:
                print("[Robust Self-Test] Error: Table is empty, cannot verify color. Test failed.")
                QApplication.instance().quit()
                return

            # 获取第一项的ID
            top_item_id = int(self.table.item(0, 9).text())

            # 模拟选择第一个常用颜色
            test_color = "#f38ba8" # 第一个常用色
            print(f"[Robust Self-Test] Applying color {test_color} to item {top_item_id}.")
            self._apply_color_to_items([top_item_id], test_color)

            # 直接查询数据库进行验证
            session = self.db.get_session()
            item = session.query(ClipboardItem).get(top_item_id)
            session.close()

            if item and item.custom_color == test_color:
                print("[Robust Self-Test] Verification PASSED: Color successfully set in DB.")
                QTimer.singleShot(500, step_5_verify_color_clear)
            else:
                print(f"[Robust Self-Test] Verification FAILED: Color in DB is '{item.custom_color if item else 'N/A'}', expected '{test_color}'.")
                QApplication.instance().quit()

        def step_5_verify_color_clear():
            print("[Robust Self-Test] Step 5: Verifying color clearing.")
            top_item_id = int(self.table.item(0, 9).text())

            print(f"[Robust Self-Test] Clearing color from item {top_item_id}.")
            self._apply_color_to_items([top_item_id], "") # 传入空字符串模拟清除

            session = self.db.get_session()
            item = session.query(ClipboardItem).get(top_item_id)
            session.close()

            if item and item.custom_color is None:
                print("[Robust Self-Test] Verification PASSED: Color successfully cleared in DB.")
                QTimer.singleShot(500, step_6_cleanup_and_exit)
            else:
                print(f"[Robust Self-Test] Verification FAILED: Color in DB is '{item.custom_color if item else 'N/A'}', expected None.")
                QApplication.instance().quit()

        def step_6_cleanup_and_exit():
            print("[Robust Self-Test] Step 6: Cleaning up and exiting.")
            if hasattr(self, 'test_file_path') and os.path.exists(self.test_file_path):
                os.remove(self.test_file_path)
            print("--- [Robust Self-Test] All test sequences completed successfully. Exiting. ---")
            QApplication.instance().quit()
    ...
    def _apply_color_to_items(self, item_ids, color_hex):
        """将颜色应用到数据库的核心逻辑"""
    ...
    def set_custom_color(self, item_ids):
        ...
        if dialog.exec_() == QDialog.Accepted:
            self._apply_color_to_items(item_ids, dialog.selected_color)
