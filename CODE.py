...
class ClipboardApp(QMainWindow):
    ...
    def init_title_bar(self):
        ...
        self.btn_auto_delete.setToolTip("清理旧数据")
        ...
        self.btn_set_color.setFixedSize(30, 30)
        self.btn_set_color.setToolTip("为选中项设置颜色")
        self.btn_set_color.clicked.connect(self.open_color_dialog_for_selection)
        self.title_bar_layout.addWidget(self.btn_set_color)
    ...
    def insert_row(self, item, idx):
        ...
        # --- 新的颜色标签显示逻辑 ---
        color_to_display = item.custom_color or item.group_color
        if color_to_display:
            cell_widget = QWidget()
            ...
            color_label = QLabel("●")
            color_label.setStyleSheet(f"color: {color_to_display}; font-size: 16px;")
            layout.addWidget(color_label)

            status_label = QLabel(status)
            layout.addWidget(status_label)

            self.table.setCellWidget(r, 1, cell_widget)
        else:
    ...
    def show_context_menu(self, pos):
        ...
        self.populate_color_menu(color_menu, item_ids)
    ...
    def populate_color_menu(self, menu, item_ids):
        """填充颜色设置的子菜单"""
        # 1. 自定义颜色
        action_custom = menu.addAction("自定义颜色...")
        action_custom.triggered.connect(lambda: self.set_custom_color(item_ids))

        # 2. 清除颜色
        action_clear = menu.addAction("清除颜色")
        action_clear.triggered.connect(lambda: self.set_predefined_color(item_ids, None))

        menu.addSeparator()

        # 3. 常用颜色子菜单
        common_colors_menu = menu.addMenu("常用颜色")
        common_colors = {
            "薄荷绿": "#98e0b3", "婴儿粉": "#f5bde6", "薰衣草紫": "#cba6f7",
            "天空蓝": "#89b4fa", "柠檬黄": "#f9e2af", "珊瑚红": "#fab387",
        }

        for name, color in common_colors.items():
            action = common_colors_menu.addAction(name)
            action.setIcon(self.create_color_icon(color))
            action.triggered.connect(lambda _, c=color: self.set_predefined_color(item_ids, c))

        # 4. 历史颜色子菜单
        history_colors_menu = menu.addMenu("历史颜色")
        recent_colors = self.get_recent_colors()

        if not recent_colors:
            action_empty = history_colors_menu.addAction("暂无历史颜色")
    ...
    def set_predefined_color(self, item_ids, color_hex):
        """为多个项目设置预定义的颜色"""
        ...
        try:
            for item_id in item_ids:
                item = session.query(ClipboardItem).get(item_id)
                if item:
                    item.custom_color = color_hex
            session.commit()
            self.add_recent_color(color_hex) # 添加到历史记录
        finally:
            session.close()
    ...
    def open_color_dialog_for_selection(self):
        """为当前选中的项目打开颜色选择对话框"""
        selected_rows = self.table.selectionModel().selectedRows()
        if not selected_rows:
            QMessageBox.information(self, "提示", "请至少选择一个项目来设置颜色。")
            return
    ...
    def get_recent_colors(self):
        """从QSettings获取最近使用的颜色列表"""
        settings = QSettings("ClipboardApp", "ColorHistory")
        return settings.value("recent_colors", [], type=list)
    ...
    def add_recent_color(self, color_hex):
        ...
        # 保持列表最多只有5个颜色
        while len(recent_colors) > 5:
            recent_colors.pop()

        settings.setValue("recent_colors", recent_colors)
