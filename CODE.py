...
class ClipboardApp(QMainWindow):
    ...
    def init_top_bar(self):
        ...
        btn_refresh = QPushButton("刷新"); btn_refresh.clicked.connect(lambda: self.load_data())
        layout.addWidget(btn_refresh)

        # 自动删除按钮
        btn_auto_delete = QPushButton("🗑️")
        btn_auto_delete.clicked.connect(self.auto_delete_old_items)
        layout.addWidget(btn_auto_delete)

        # 模式切换按钮
        self.mode_btn = QPushButton("📖 读取模式")
    ...
    def show_context_menu(self, pos):
        ...
        # 1. 常用颜色
        common_colors_menu = color_menu.addMenu("常用颜色")
        common_colors = ["#ffadad", "#ffd6a5", "#caffbf", "#9bf6ff", "#a0c4ff", "#bdb2ff"]
        for color in common_colors:
            action = common_colors_menu.addAction(color)
            action.setIcon(self.create_color_icon(color))
            action.triggered.connect(lambda _, c=color, ids=item_ids: self.set_custom_color(ids, c))

        # 2. 历史颜色
        history_menu = color_menu.addMenu("历史颜色")
        settings = QSettings("ClipboardApp", "ColorHistory")
        history = settings.value("colors", [])
        if history:
            for color in history[:6]: # 最多显示6个
                action = history_menu.addAction(color)
                action.setIcon(self.create_color_icon(color))
                action.triggered.connect(lambda _, c=color, ids=item_ids: self.set_custom_color(ids, c))

        color_menu.addSeparator()
        color_menu.addAction("更多颜色...").triggered.connect(lambda: self.set_custom_color(item_ids))
        color_menu.addAction("清除颜色").triggered.connect(lambda: self.set_custom_color(item_ids, ""))
    ...
    def toggle_edit_mode(self):
        ...
        if self.edit_mode:
            self.mode_btn.setText("✏️ 编辑模式")
            self.table.setEditTriggers(QAbstractItemView.DoubleClicked)
            self.table.setDragEnabled(True)  # 编辑模式允许拖拽
        else:
            self.mode_btn.setText("📖 读取模式")
    ...
    def set_custom_color(self, item_ids, color_hex=None):
        ...
        try:
            for item_id in item_ids:
                item = session.query(ClipboardItem).get(item_id)
                if item:
                    item.custom_color = color_hex if color_hex else None # 空字符串存为None
            session.commit()
        finally:
            session.close()
        self.load_data(select_id=item_ids[0] if item_ids else None)
