...
class AICodeEditor:
    ...
    def create_ui(self):
        ...
        # 右侧代码编辑区
        code_frame = ttk.LabelFrame(top_pane, text="代码编辑区", padding=5)
        top_pane.add(code_frame, weight=4)
        ...
        # 下方面板: 标签页
        bottom_pane = ttk.Frame(main_container, padding=(5, 0))
        main_container.add(bottom_pane, weight=1)

        self.bottom_notebook = ttk.Notebook(bottom_pane)
        self.bottom_notebook.pack(fill=tk.BOTH, expand=True)

        # -- 输出控制台标签页 --
        console_tab = ttk.Frame(self.bottom_notebook)
        self.bottom_notebook.add(console_tab, text="输出控制台")

        self.console_text = scrolledtext.ScrolledText(console_tab, wrap=tk.WORD, font=("Consolas", 10))
        ...
        # -- AI助手标签页 --
        ai_tab = ttk.Frame(self.bottom_notebook)
        self.bottom_notebook.add(ai_tab, text="AI助手")

        self.chat_display = scrolledtext.ScrolledText(ai_tab, wrap=tk.WORD, font=("Arial", 10), state=tk.DISABLED)
