...
class TagSelectorDialog(QDialog):
    ...
    def load_tags(self):
        ...
        for name, count in filtered_tags:
            ...
            item.setData(Qt.UserRole, name)  # 存储标签名
    ...
    def switch_view(self, mode):
        ...
        # TODO: 实现网格视图
        if mode == "grid":
            QMessageBox.information(self, "提示", "网格视图功能开发中...")
...
class ClipboardApp(QMainWindow):
    ...
    def init_table(self):
        ...
        w(0, 40); w(1, 80); w(2, 120); w(3, 60); w(4, 70); w(5, 40); w(6, 140); w(7, 140)
    ...
    def insert_row(self, item, idx):
        ...
        # 序号
        seq_item = QTableWidgetItem(str(idx)); seq_item.setTextAlignment(Qt.AlignCenter)
        ...
        # 备注
        note_item = QTableWidgetItem(item.note); self.table.setItem(r, 2, note_item)

        # 星级
        stars = "★" * item.star_level if item.star_level > 0 else ""
        star_item = QTableWidgetItem(stars); star_item.setTextAlignment(Qt.AlignCenter); star_item.setForeground(QColor("#FFD700"))
        self.table.setItem(r, 3, star_item)

        # 大小
        size_item = QTableWidgetItem(self.format_size(item.content)); size_item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
        self.table.setItem(r, 4, size_item)

        # 访问
        visit_item = QTableWidgetItem(str(item.visit_count)); visit_item.setTextAlignment(Qt.AlignCenter)
        self.table.setItem(r, 5, visit_item)

        # 创建时间
        created_item = QTableWidgetItem(item.created_at.strftime("%Y-%m-%d %H:%M:%S")); self.table.setItem(r, 6, created_item)
        ...
        # ID(隐藏)
        id_item = QTableWidgetItem(str(item.id)); self.table.setItem(r, 9, id_item)

        # 颜色标记 (点) - 优先级: 自定义 > 分组
        color_to_apply = item.custom_color or item.group_color
        if color_to_apply:
            pixmap = QPixmap(16, 16); pixmap.fill(Qt.transparent)
            painter = QPainter(pixmap); painter.setRenderHint(QPainter.Antialiasing)
            painter.setBrush(QColor(color_to_apply)); painter.setPen(Qt.NoPen)
            painter.drawEllipse(2, 5, 6, 6); painter.end()
    ...
    def show_context_menu(self, pos):
        ...
        menu = QMenu(); menu.setStyleSheet("QMenu { background-color: #313244; color: white; border: 1px solid #45475a; }")
        ...
        menu.addSeparator()
        color_menu = menu.addMenu(f"🎨 设置颜色 ({len(item_ids)}项)")
        color_menu.addAction("随机分组").triggered.connect(lambda: self.set_group_color(item_ids))
        color_menu.addAction("自定义...").triggered.connect(lambda: self.set_custom_color(item_ids))
        color_menu.addSeparator()
        color_menu.addAction("清除颜色").triggered.connect(lambda: self.clear_color(item_ids))
    ...
    def set_column_alignment(self, col, alignment):
        ...
        settings = QSettings("ClipboardApp", "WindowState"); settings.setValue(f"column_{col}_alignment", int(alignment))
    ...
    def batch_delete(self, item_ids):
        reply = QMessageBox.question(self, "确认删除", f"确定要删除 {len(item_ids)} 个项目吗?", QMessageBox.Yes | QMessageBox.No)
        ...
        try:
            for item_id in item_ids:
                item = session.query(ClipboardItem).get(item_id)
                if item and not item.is_locked: session.delete(item)
            session.commit()
        finally: session.close()
        self.load_data()
    ...
    def toggle_edit_mode(self):
        ...
        self.mode_btn_title.setChecked(self.edit_mode)
        if self.edit_mode:
            self.mode_btn_title.setText("✏️")
            self.mode_btn_title.setToolTip("切换为读取模式")
            self.table.setEditTriggers(QAbstractItemView.DoubleClicked)
            self.table.setDragEnabled(True)
        else:
            self.mode_btn_title.setText("📖")
            self.mode_btn_title.setToolTip("切换为编辑模式")
    ...
    def del_tag(self, name):
        if hasattr(self, 'current_id'):
            ...
            self.update_dock_panel(); self.refresh_tag_cloud()
    ...
    def group_selected_items(self):
        ...
        item_ids = [int(self.table.item(row.row(), 9).text()) for row in selected_rows]
        self.set_group_color(item_ids)

    def set_group_color(self, item_ids):
        ...
        self.load_data(select_id=item_ids[0])

    def clear_color(self, item_ids):
        if not item_ids: return
        session = self.db.get_session()
        try:
            items = session.query(ClipboardItem).filter(ClipboardItem.id.in_(item_ids)).all()
            for item in items:
                item.custom_color = None; item.group_color = None
            session.commit()
        finally: session.close()
        self.load_data(select_id=item_ids[0])

    def set_custom_color(self, item_ids, color_hex=None):
        if not item_ids: return
        ...
        if color_hex == "": self.clear_color(item_ids); return

        session = self.db.get_session()
        try:
            items = session.query(ClipboardItem).filter(ClipboardItem.id.in_(item_ids)).all()
            for item in items: item.custom_color = color_hex
            session.commit()
        finally: session.close()
        self.load_data(select_id=item_ids[0])

    def set_color_action(self):
        selected_rows = self.table.selectionModel().selectedRows()
        if not selected_rows:
            QMessageBox.information(self, "提示", "请至少选择一个项目")
            return
        item_ids = [int(self.table.item(row.row(), 9).text()) for row in selected_rows]
        self.set_custom_color(item_ids)

    def generate_unique_color(self):
        color_palette = ["#2d3748", "#2c5282", "#2f855a", "#744210", "#5a2e5e", "#2c5f5f", "#4a5568", "#5a3825", "#2d4a3e", "#3d3846", "#2e4057", "#4a3f35"]
        available_colors = [c for c in color_palette if c not in self.used_colors]
        if not available_colors: self.used_colors.clear(); available_colors = color_palette
        import random
        color = random.choice(available_colors); self.used_colors.add(color)
        return color

    def auto_delete_old_items(self):
        reply = QMessageBox.question(self, "确认清理", "确定要删除21天前的数据吗?\n(已锁定的数据不会被删除)", QMessageBox.Yes | QMessageBox.No)
    ...
    def restore_window_state(self):
        ...
        if filter_type == "favorite": self.btn_fav.setChecked(True); self.btn_all.setChecked(False); self.current_filter = "favorite"
        elif filter_type == "locked": self.btn_lock.setChecked(True); self.btn_all.setChecked(False); self.current_filter = "locked"

        sort_index = settings.value("sortIndex", 0)
        if sort_index: self.sort_combo.setCurrentIndex(int(sort_index))

        column_widths = settings.value("columnWidths")
        if column_widths:
            for i, width in enumerate(column_widths):
                if i < self.table.columnCount(): self.table.setColumnWidth(i, int(width))

    def closeEvent(self, event): self.save_window_state(); event.accept()

    def init_title_bar(self):
        self.title_bar = QFrame(); self.title_bar.setFixedHeight(32); self.title_bar.setObjectName("TitleBar")
        self.title_bar.setStyleSheet("#TitleBar { background-color: #1e1e2e; border-top-left-radius: 8px; border-top-right-radius: 8px; }")
        title_layout = QHBoxLayout(self.title_bar); title_layout.setContentsMargins(10, 0, 0, 0); title_layout.setSpacing(10)

        title_label = QLabel("印象记忆"); title_label.setStyleSheet("color: #cdd6f4; font-weight: bold;")
        title_layout.addWidget(title_label)
        title_layout.addStretch()

        self.btn_refresh_title = QPushButton(QIcon.fromTheme("view-refresh"), ""); self.btn_refresh_title.setObjectName("TitleBarButton")
        self.btn_refresh_title.setToolTip("刷新"); self.btn_refresh_title.clicked.connect(lambda: self.load_data())
        title_layout.addWidget(self.btn_refresh_title)

        self.btn_auto_delete_title = QPushButton("🗑️"); self.btn_auto_delete_title.setObjectName("TitleBarButton")
        self.btn_auto_delete_title.setToolTip("清理旧数据"); self.btn_auto_delete_title.clicked.connect(self.auto_delete_old_items)
        title_layout.addWidget(self.btn_auto_delete_title)

        self.btn_set_color_title = QPushButton("🎨"); self.btn_set_color_title.setObjectName("TitleBarButton")
        self.btn_set_color_title.setToolTip("设置颜色"); self.btn_set_color_title.clicked.connect(self.set_color_action)
        title_layout.addWidget(self.btn_set_color_title)

        self.mode_btn_title = QPushButton("📖"); self.mode_btn_title.setCheckable(True); self.mode_btn_title.setObjectName("TitleBarButton")
        self.mode_btn_title.setToolTip("切换编辑/读取模式"); self.mode_btn_title.clicked.connect(self.toggle_edit_mode)
        title_layout.addWidget(self.mode_btn_title)

        btn_minimize = QPushButton("-"); btn_maximize = QPushButton("[]"); btn_close = QPushButton("X")
        btn_minimize.setObjectName("TitleBarButton"); btn_maximize.setObjectName("TitleBarButton"); btn_close.setObjectName("TitleBarButton")
        btn_minimize.setToolTip("最小化"); btn_maximize.setToolTip("最大化"); btn_close.setToolTip("关闭")
        btn_minimize.clicked.connect(self.showMinimized); btn_maximize.clicked.connect(self.toggle_maximize); btn_close.clicked.connect(self.close)
        title_layout.addWidget(btn_minimize); title_layout.addWidget(btn_maximize); title_layout.addWidget(btn_close)

        self.central_layout.insertWidget(0, self.title_bar)
        self.oldPos = None
